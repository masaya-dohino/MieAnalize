...
const int Nx = 1019;//64;//64;//80;//128;
const int Ny = 1019;//64;//64;//80;//128;
...
void keyboard( unsigned char key, int x, int y )
{
    ...
    switch ( key ) {
		...
		case 'p':
			...
			if(mode == TM_WGM){
				filename_len = sprintf_s(filename, 100, "C:\\Users\\zh\\Desktop\\Œ¤‹†\\code from kandasan\\MieAnalyzer\\MieAnalyzer\\outputFile\\TM_WGM_MIE_SOLUTION_Ez_Intensity_rad%3.1f_lam%3.1f_coef%2.1f.txt", radius, lam, coef);
			}
			else if( mode == TE_WGM ){
				filename_len = sprintf_s(filename, 100, "C:\\Users\\zh\\Desktop\\Œ¤‹†\\code from kandasan\\MieAnalyzer\\MieAnalyzer\\outputFile\\TE_WGM_MIE_SOLUTION_Ey_Intensity_rad%3.1f_lam%3.1f_coef%2.1f.txt", radius, lam, coef);
			}
			else if( mode == TM_Mie ){
				//filename_len = sprintf_s(filename, 100, "C:\\Users\\zh\\Desktop\\Œ¤‹†\\code from kandasan\\MieAnalyzer\\MieAnalyzer\\outputFile\\TM_MIE_SOLUTION_Ez_Intensity_rad%3.1f_lam%3.1f_coef%2.1f.txt", radius, lam, coef);
				filename_len = sprintf_s(filename, 200, "C:\\Users\\zh\\Desktop\\Œ¤‹†\\code from kandasan\\MieAnalyzer\\MieAnalyzer\\outputFile\\TM_MIE_SOLUTION_Ez_Intensity_rad.txt");
			}
			else if( mode == TE_Mie ){
				filename_len = sprintf_s(filename, 100, "C:\\Users\\zh\\Desktop\\Œ¤‹†\\code from kandasan\\MieAnalyzer\\MieAnalyzer\\outputFile\\TE_MIE_SOLUTION_Ey_Intensity_rad%3.1f_lam%3.1f_coef%2.1f.txt", radius, lam, coef);
			}
...
*/
	...
	if(mode == TM_WGM){
		...
		fopen_s(&fp, "C:\\Users\\zh\\Desktop\\Œ¤‹†\\code from kandasan\\MieAnalyzer\\MieAnalyzer\\outputFile\\TM_WGM_MIE_SOLUTION_Ez_Field.txt", "w");
	}
	else if(mode == TE_WGM){
		//fopen_s( &fp, "../outputFile/TE_WGM_MIE_SOLUTION_Ey_Field.txt", "w" );
		fopen_s(&fp, "C:\\Users\\zh\\Desktop\\Œ¤‹†\\code from kandasan\\MieAnalyzer\\MieAnalyzer\\outputFile\\TE_WGM_MIE_SOLUTION_Ey_Field.txt", "w");
	}
	else if( mode == TM_Mie ){
		//fopen_s( &fp, "../outputFile/TM_MIE_SOLUTION_Ez_Field.txt", "w");
		fopen_s(&fp, "C:\\Users\\zh\\Desktop\\Œ¤‹†\\code from kandasan\\MieAnalyzer\\MieAnalyzer\\outputFile\\TM_MIE_SOLUTION_Ez_Field.txt", "w");
	}
	else if( mode == TE_Mie ){
		//fopen_s( &fp, "../outputFile/TE_MIE_SOLUTION_Ey_Field.txt", "w");
		fopen_s(&fp, "C:\\Users\\zh\\Desktop\\Œ¤‹†\\code from kandasan\\MieAnalyzer\\MieAnalyzer\\outputFile\\TE_MIE_SOLUTION_Ey_Field.txt", "w");
	}
	else {
		//fopen_s(&fp, "../outputFile/TM_WGM_MIE_SOLUTION_Ez_Field.txt", "w");
		fopen_s(&fp, "C:\\Users\\zh\\Desktop\\Œ¤‹†\\code from kandasan\\MieAnalyzer\\MieAnalyzer\\outputFile\\TM_WGM_MIE_SOLUTION_Ez_Field.txt", "w");
